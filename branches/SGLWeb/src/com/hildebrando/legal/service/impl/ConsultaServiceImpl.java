...
public class ConsultaServiceImpl implements ConsultaService,Serializable {
	...
	public List getPersonas() {
		...
		try {
			...
			logger.debug("Lista de personas gral sin filtro::"+personas.size());
			/*for (int i = 0; i < personas.size(); i++) {
				logger.debug("PersGral->\t"+personas.get(i).getIdPersona() + "  "+
						personas.get(i).getNumeroDocumento()+"  "+personas.get(i).getNombreCompleto());
			}*/
			return personas;
		} catch (Exception e) {
	...
	public List getPersonasByPersona(Persona persona) {
		logger.debug("== getPersonasByPersona==");
		...
		if(persona.getClase()!= null && persona.getClase().getIdClase()!=-1){			
			logger.debug("[buscPersona]-idClase: "+ persona.getClase().getIdClase()  +"");
		...
		if(persona.getTipoDocumento()!= null && persona.getTipoDocumento().getIdTipoDocumento()!=-1){			
			logger.debug("[buscPersona]-idTipoDoc:  "+ persona.getTipoDocumento().getIdTipoDocumento() +" ");
		...
		if(persona.getApellidoMaterno().compareTo("")!=0){			
			logger.debug("[buscPersona]-apMaterno: "+ persona.getApellidoMaterno()  +"");
		...
		try {
			...
			logger.debug("ConsultaService-getPersonasByPersona total:"+personas.size());
			/*for(int i=0;i<personas.size();i++){
				logger.debug("[BD]- Persona: "+personas.get(i).getIdPersona()+"  "+personas.get(i).getNumeroDocumento()+ "   "+personas.get(i).getNombreCompleto());
			}*/
