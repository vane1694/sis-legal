...
public class RegistroExpedienteMB implements Serializable {
	...
	public void buscarPersona(ActionEvent e) {
		...
		try {
			if (getIdClase() != -1 || getCodCliente() != null
				...
				if(personas!=null){
					logger.debug(SglConstantes.MSJ_TAMANHIO_LISTA+"de Persona (getPersonasByPersona) es: ["+personas.size()+"]");
				}
				...
				if(personas!=null){
					logger.debug(SglConstantes.MSJ_TAMANHIO_LISTA+"de Personas-2 es: ["+personas.size()+"]");
				}
				personaDataModelBusq = new PersonaDataModel(personas);
			}
		} catch (Exception e1) {
	...
	public void agregarInvolucrado(ActionEvent e) {
		logger.debug("==== agregarInvolucrado() ====");
		
		if(involucrado.getPersona() != null){
			logger.debug("involucrado.Persona.Id: "+involucrado.getPersona().getIdPersona());
			logger.debug("involucrado.Persona.NroDoc: "+involucrado.getPersona().getNumeroDocumento());
			logger.debug("involucrado.Persona.TipoDoc: "+involucrado.getPersona().getTipoDocumento().getIdTipoDocumento());
			logger.debug("involucrado.Persona.Clase: "+involucrado.getPersona().getClase().getIdClase());
			logger.debug("involucrado.Persona.Nombres: "+involucrado.getPersona().getNombres());
			logger.debug("involucrado.Persona.ApPat: "+involucrado.getPersona().getApellidoPaterno()!=null?involucrado.getPersona().getApellidoPaterno():" ");
			logger.debug("involucrado.Persona.ApMat: "+involucrado.getPersona().getApellidoMaterno()!=null?involucrado.getPersona().getApellidoMaterno():" ");
			
		}else{
			logger.debug("No llega la persona ..es NULA");
		}
	...
	public void agregarPersona(ActionEvent e) {
		...
		logger.info("==== agregarPersona v2_() =====");
		if(!validarPersona()){
			...
			tdoc.setIdTipoDocumento(getIdTipoDocumento());
			per.setNumeroDocumento(getNumeroDocumento());
			per.setNombres(getTxtNombres()!=null?getTxtNombres():"");
			per.setApellidoMaterno(getTxtApellidoMaterno()!=null?getTxtApellidoMaterno():"");
			per.setApellidoPaterno(getTxtApellidoPaterno()!=null?getTxtApellidoPaterno():"");
			per.setClase(cls);
			per.setTipoDocumento(tdoc);			
			
			List<Persona> personas = new ArrayList<Persona>();

			//13.05.2014 Implementando busqueda por Tipo y Nro Doc
			...
			filtro.add(Restrictions.eq("tipoDocumento.idTipoDocumento",	per.getTipoDocumento().getIdTipoDocumento()));
			filtro.add(Restrictions.eq("numeroDocumento", per.getNumeroDocumento()));
			logger.debug("[existePersona]-idTipoDocumento:"+per.getTipoDocumento().getIdTipoDocumento());
			logger.debug("[existePersona]-nroDocumento:"+per.getNumeroDocumento());
			
			try {
				personas = personaDAO.buscarDinamico(filtro);
			} catch (Exception e3) {
				logger.error("Ha ocurrido un error al buscar la persona en BD: ",e3);
			}
			...
			if (personas.size() == 0) {
				logger.debug("La persona no existe en BD, se procedera a registrarlo.");
				try {
				...
				logger.debug("No existe la persona con los filtros.");
				FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_INFO,
	...
	//Metodo que valida el ingreso de datos en el popup de inculpado
		public boolean validarInculpado(){
			boolean res=true;
			String msjValidacionInc="";
			if(getIdClase_inclp() == -1){
				logger.debug("[validaInculpado]-No ha seleccionado la Clase");
				msjValidacionInc = msjValidacionInc + "-Clase ";
				res=false;
			}		
			if(getIdTipoDocumento_inclp() == -1){
				logger.debug("[validaInculpado]-No ha seleccionado un Tipo de Documento");
				msjValidacionInc = msjValidacionInc + "-Tipo Doc. ";
				res=false;
			}
			//Si es DNI
			if(getIdTipoDocumento_inclp()==1){
				//Se registran datos personales
				if(getTxtApellidoMaterno_inclp().equalsIgnoreCase("")){
					logger.debug("[validaInculpado]-El getTxtApellidoMaterno() es VACIO");
					msjValidacionInc = msjValidacionInc + "-Ap.Materno ";
					res=false;
				}
				if(getTxtApellidoPaterno_inclp().equalsIgnoreCase("")){
					logger.debug("[validaInculpado]-El getTxtApellidoPaterno() es VACIO");
					msjValidacionInc = msjValidacionInc + "-Ap.Paterno ";
					res=false;
				}
			}		
			
			if(String.valueOf(getNumeroDocumento_inclp()).length()<8 || getNumeroDocumento_inclp() == 0){
				logger.debug("[validaInculpado]Ingrese NRO mayor a 8 digitos");
				msjValidacionInc = msjValidacionInc + "-Nro Doc. ";
				res=false;
			}
			
			if(getTxtNombres_inclp().equalsIgnoreCase("") || getTxtNombres_inclp().trim().length()<2){
				logger.debug("[validaInculpado]-El getTxtNombres() es VACIO");
				msjValidacionInc = msjValidacionInc + "-Nombres ";
				res=false;			
			}
			if(!res){
				FacesMessage msg = new FacesMessage(
						FacesMessage.SEVERITY_INFO,"Datos requeridos: "+msjValidacionInc,"");
				FacesContext.getCurrentInstance().addMessage(null, msg);
			}
			return res;
		}

	//Metodo para agregar inculpado en BD
	public void agregar_Inculpado(ActionEvent e) {
		logger.debug("=== agregar_Inculpado() ===");
		if(!validarInculpado()){
			logger.debug("[agregInculp]-No ha pasado la validacion, por lo que no se puede registrar.");
			...
			logger.debug("Se procede a registrar el inculpado-persona");
			Persona per = new Persona();
			Clase cls = new Clase();
			cls.setIdClase(getIdClase_inclp());
			per.setCodCliente(getCodCliente_inclp());
			...
			per.setClase(cls);
			per.setNumeroDocumento(getNumeroDocumento_inclp());
			per.setNombres(getTxtNombres_inclp()==null?"":getTxtNombres_inclp());
			per.setApellidoMaterno(getTxtApellidoMaterno_inclp()==null?"":getTxtApellidoMaterno_inclp());
			per.setApellidoPaterno(getTxtApellidoPaterno_inclp()==null?"":getTxtApellidoPaterno_inclp());
			...
			filtro.add(Restrictions.eq("tipoDocumento.idTipoDocumento",	per.getTipoDocumento().getIdTipoDocumento()));
			filtro.add(Restrictions.eq("numeroDocumento", per.getNumeroDocumento()));
			logger.debug("[existeInculpad]-idTipoDocumento:"+per.getTipoDocumento().getIdTipoDocumento());
			logger.debug("[existeInculpad]-nroDocumento:"+per.getNumeroDocumento());
			...
			if (personas.size() == 0) {
				try {
					logger.debug("Aun no existe esta persona, se procedera a registrar el inculpado.");
					per.setNombreCompleto(per.getNombres() + " "
							+ per.getApellidoPaterno() + " "
							+ per.getApellidoMaterno());
					
					//20140512 - Se agregan datos faltantes
					per.setFechaCreacion(new Date());
					per.setFechaModificacion(new Date());
					per.setUsuarioCreacion(getResponsable().getCodigo()!=null?
							getResponsable().getCodigo():"BBVA");
					...
					personabd = personaService.registrar(per);
					logger.debug("Se ha registrado la persona: "+per.getNombres());
					FacesMessage msg = new FacesMessage(
							FacesMessage.SEVERITY_INFO, "Persona agregada",	"Persona agregada");
					FacesContext.getCurrentInstance().addMessage(null, msg);

				} catch (Exception e2) {
					logger.error("Ha ocurrido un error al registrar el inculpado: ",e2);
				}
			} else {
				FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_INFO,
						"Persona Existente", "Persona Existente");
			...
			/* 15.03.2014 Se comenta esta parte
			 * List<Persona> personas2 = new ArrayList<Persona>();
			personas2.add(personabd);
			personaDataModelBusq = new PersonaDataModel(personas2);*/
	...
	public void seleccionarPersona() {
		...
		try{
			...
			logger.debug("[Persona]-getInvol-Id: "+getInvolucrado().getPersona().getIdPersona());
		
		}catch (Exception e) {
			logger.error("Ha ocurrido un error en seleccionarPersona-REG: ", e);
		}
		
		
		logger.debug("== saliendo de seleccionarPersona() ==");
	}
...
//			}
	...
	public List<Persona> completePersona(String query) {
		...
		if (personas != null) {
			logger.debug(SglConstantes.MSJ_TAMANHIO_LISTA + "personas en completePersona() es:["+ personas.size() + "]. ");
		}
		...
		for (Persona pers : personas) {
			...
			if (nombreCompletoMayuscula.contains(query.toUpperCase())) {
				...
				//logger.debug("valor autocompletable: "+pers.getNombreCompletoMayuscula());
				results.add(pers);
			}
		}
		return results;
