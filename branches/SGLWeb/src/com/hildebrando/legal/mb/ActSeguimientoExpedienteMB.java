...
public class ActSeguimientoExpedienteMB {
	...
	public void selectFechaDatatable(DateSelectEvent event) {
		...
		if(!esValido(date)){			
			//getExpedienteVista().getActividadProcesal().setFechaActividadAux(null);
			logger.debug("selectFechaDatatable-Fecha invalida");
		}		
		esValidoMsj(date);
	}
	...
	public void esValidoMsj(Date date) {
		...
		if (flagDomingo == true) {
			FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_INFO,	"Información", "Es domingo escoger otra fecha !");
			FacesContext.getCurrentInstance().addMessage(null, msg);
		} else {
			if (flagFeriado == true) {
				FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_INFO,	"Información", "Es feriado escoger otra fecha !");
				FacesContext.getCurrentInstance().addMessage(null, msg);
			}
		}
	}
	...
	public void crearProximaInstancia(ActionEvent e) 
	{
		...
		try {
			...
			if(instanciaProximaBD!=null){
				logger.debug("[crearProxInst]-Id Instancia:"+instanciaProximaBD.getIdInstancia());
			}
			expedienteSiguiente.setInstancia(instanciaProximaBD);
		} catch (Exception e2) {
			logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"instancias-crearProxInst: ",e2);
		}
		
		try {
			logger.debug("[crearProxInst]-expedienteSiguiente " +expedienteSiguiente.toString());
		...
		try {
			...
			logger.error(SglConstantes.MSJ_ERROR_ACTUALIZ+"el expedienteSiguiente:",ex);
		}
	...
	/**
	 * Metodo encargado de guardar el expediente modificado 
	 * (actualizar en BD) al hacer click en el boton 'Guardar'
	 * @param e Evento del tipo {@link ActionEvent}
	 * **/
	public void actualizar(ActionEvent e) {
		logger.debug("======== actualizar() ======");
		logger.debug("[actualizExp]-Component: " + e.getComponent().getId());
...
//				}
				...
				int total = obtenerTotalCuotasxExpediente(expediente, hno);				
				if (total>0){
					...
					if (total!=hno.getTotalCuotas() || hno.getMonto().compareTo(totalHonor)!=0)
					{
						...
						{
							//1.- Verifica la situacion del honorario que no este pagado. Si lo esta se elimina todos las cuotas de ese ID Honorario sino solo las pendientes
							
							if (hno.getSituacionHonorario().getDescripcion().trim().equals(SglConstantes.SITUACION_HONORARIO_PAGADO))
							{
								eliminarListasCuotasSitPagado(expediente,hno);
								eliminarHonorario(expediente,hno);
							}
							else{
								//Eliminar cuotas
								boolean isEdit=false;
...
//					}
			...
			if (hno!=null)
			{
				logger.debug("[actualizExp]-Total de cuotas del expediente: " + hno.getCuotas().size());
			}
			else
			{
		...
		try {
			...
			logger.debug(SglConstantes.MSJ_EXITO_ACTUALIZ+"el expediente.");
						...
						if (getExpedienteVista().getHonorario().getMonto() == 0.0) {
							...
							if (getExpedienteVista().getHonorario().getSituacionHonorario().getDescripcion() == "") {
								...
								try {
									...
									logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"abogadoEstudios: "+e);
								}
				...
				if (anexo.getComentario() == "") {
					...
					if (anexo.getFechaInicio() == null) {
						...
						try {
							...
							logger.error("error anexo " + e.toString());
						} finally {
							...
							if (canalSalida != null) {
								try {
									...
									logger.error(SglConstantes.MSJ_ERROR_EXCEPTION+"canalSalida-anexo:"+x);
								}
							}
						}
...
//			resultadoflo = restarDomingos(resultadoflo, validarDomingo);
	...
	public void seleccionarAbogado() {
		getSelectedAbogado().setNombreCompletoMayuscula(
				...
				.concat(" ").concat(getSelectedAbogado().getApellidoPaterno()!=null?getSelectedAbogado().getApellidoPaterno().toUpperCase():"")
				.concat(" ").concat(getSelectedAbogado().getApellidoMaterno()!=null?getSelectedAbogado().getApellidoMaterno().toUpperCase():""));
	...
	public void seleccionarInvolucrado() {
		...
		.concat("").concat(getSelectInvolucrado().getApellidoPaterno()!=null?getSelectInvolucrado().getApellidoPaterno().toUpperCase():"")
		.concat("").concat(getSelectInvolucrado().getApellidoMaterno()!=null?getSelectInvolucrado().getApellidoMaterno().toUpperCase():""));
	...
	//Metodo que valida el ingreso de datos en el popup de persona
		public boolean validarPersona(){
			boolean res=true;
			String msjValidacion="";
			if(getPersona().getClase().getIdClase() == 0 || 
					String.valueOf(getPersona().getClase().getIdClase()).equalsIgnoreCase("")){
				logger.debug("[validaPersonaEdit]-No ha seleccionado la Clase");
				msjValidacion = msjValidacion + "-Clase ";
				res=false;
			}		
			if(getPersona().getTipoDocumento().getIdTipoDocumento() == 0 || 
					String.valueOf(getPersona().getTipoDocumento().getIdTipoDocumento()).equalsIgnoreCase("")){
				logger.debug("[validaPersonaEdit]-No ha seleccionado un Tipo de Documento");
				msjValidacion = msjValidacion + "-Tipo Doc. ";
				res=false;
			}
			//Si es DNI
			if(getPersona().getTipoDocumento().getIdTipoDocumento()==1){
				//Se registran datos personales
				if(getPersona().getApellidoMaterno().equalsIgnoreCase("")){
					logger.debug("[validaPersonaEdit]-El getTxtApellidoMaterno() es VACIO");
					msjValidacion = msjValidacion + "-Ap.Materno ";
					res=false;
				}
				if(getPersona().getApellidoPaterno().equalsIgnoreCase("")){
					logger.debug("[validaPersonaEdit]-El getTxtApellidoPaterno() es VACIO");
					msjValidacion = msjValidacion + "-Ap.Paterno ";
					res=false;
				}
			}
			if(String.valueOf(getPersona().getNumeroDocumento()).length()<8 || getPersona().getNumeroDocumento() == 0){
				logger.debug("[validaPersonaEdit]-Ingrese NRO mayor a 8 digitos");
				msjValidacion = msjValidacion + "-Nro Doc. ";
				res=false;
			}
			
			if(getPersona().getNombres().equalsIgnoreCase("") || getPersona().getNombres().trim().length()<2){
				logger.debug("[validaPersonaEdit]-Ingrese el nombre, no debe ser VACIO");
	...
	public void agregarPersona(ActionEvent e) {
		logger.info("=== editar - agregarPersonav2() ===");
		if(!validarPersona()){
			logger.debug("No se puede actualizar porque no cumple con las validaciones");
			/*if (getPersona().getClase().getIdClase() == 0
			...
			logger.debug("Pasaron las validaciones, se registrara la persona.");
			List<Persona> personas = new ArrayList<Persona>();
			GenericDao<Persona, Object> personaDAO = (GenericDao<Persona, Object>) SpringInit.getApplicationContext().getBean("genericoDao");
			Busqueda filtro = Busqueda.forClass(Persona.class);
			/*filtro.add(Restrictions.eq("clase.idClase", getPersona().getClase().getIdClase()));
			...
			try {
				...
				logger.error("Ha ocurrido un error al buscar la persona: ",e3);
			}

			Persona personabd = new Persona();

			if (personas.size() == 0) {
				logger.debug("La persona no existe, se procedera a registrarla.");
				try {
					//13.05.2014 Se agrega
					Persona pers = new Persona();
					Clase cls = new Clase();
					TipoDocumento tdoc = new TipoDocumento();
					cls.setIdClase(getPersona().getClase().getIdClase());
					pers.setCodCliente(getPersona().getCodCliente());
					tdoc.setIdTipoDocumento(getPersona().getTipoDocumento().getIdTipoDocumento());
					pers.setTipoDocumento(tdoc);
					pers.setClase(cls);
					pers.setNumeroDocumento(getPersona().getNumeroDocumento());
					pers.setNombres(getPersona().getNombres()==null?"":getPersona().getNombres());
					pers.setApellidoMaterno(getPersona().getApellidoMaterno()==null?"":getPersona().getApellidoMaterno());
					pers.setApellidoPaterno(getPersona().getApellidoPaterno()==null?"":getPersona().getApellidoPaterno());
					pers.setNombreCompleto(pers.getNombres() + " "+ pers.getApellidoPaterno() + " "+ pers.getApellidoMaterno());
					pers.setFechaCreacion(new Date());
					pers.setFechaModificacion(new Date());
					pers.setUsuarioCreacion("BBVA");
					pers.setUsuarioModificacion("BBVA");
					
					/*getPersona().setNombreCompleto(getPersona().getNombres() + " " + getPersona().getApellidoPaterno() + " " + getPersona().getApellidoMaterno());
					getPersona().setFechaCreacion(new Date());
					getPersona().setFechaModificacion(getPersona().getFechaCreacion());
					getPersona().setUsuarioCreacion("BBVA"); //TODO Incluir usuario sesion
					getPersona().setUsuarioModificacion("BBVA"); //TODO incluir usuario sesion
					*/
					
					personabd = personaDAO.insertar(pers); //getPersona()
					logger.debug("Se ha registrado la personaAct: "+pers.getNombres() +"correctamente.");
					FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "Persona agregada",	"Persona agregada");
					FacesContext.getCurrentInstance().addMessage(null, msg);

				} catch (Exception e2) {
					logger.error("Ha ocurrido un error al registrar la persona: ",e2);
				}
			...
			//13.05.2014 Se comenta estas 3 lineas
	...
	public void actualizarExpedienteActual(Expediente expediente,ExpedienteVista expedienteVista) {
		if (isFlagGuardarInstancia()) {
			...
			try {
				...
				logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"instanciabd: "+e);
			}
		...
		logger.debug("[editExp]-Proceso: "+expedienteVista.getProceso());
		logger.debug("[editExp]-isFlagModificadoInc(): "+isFlagModificadoInc());
		
		//if (expedienteVista.getProceso() != 2 && isFlagModificadoInc()) {
		if (isFlagModificadoInc()) {
			logger.debug("=== inculpados ===");
			List<Inculpado> inculpados = expedienteVista.getInculpados();
			expediente.setInculpados(new ArrayList<Inculpado>());
			logger.debug("Lista de inculpados: "+inculpados.size());
		...
		if (isFlagGuardarMoneda()) {
			...
			try {
				...
				logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"monedabd: "+e);
			}
		}
		...
		if (isFlagGuardarTipoMediaCautelar()) {
			...
			try {
				...
				logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"tipoCautelarbd: "+e);
			}
		}
		...
		if (isFlagGuardarContraCautela()) {
			...
			try {
				...
				logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"contraCautelabd: "+e);
			}
		}
		...
		if (isFlagGuardarEstado()) {
			...
			try {
				...
				logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"estadoCautelarbd: "+e);
			}
		}
						...
						if (anexo != null) 
						{
							if (anexo.getIdDocumento() == 0) {
								...
								try {
									...
									logger.error(SglConstantes.MSJ_ERROR_ACTUALIZ+"anexo-canalSalida: "+e);
								}
		...
		if (isFlagGuardarRiesgo()) 
		{
			...
			try {
				...
				logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"riesgobd: "+e);
			}
		}
	}
	...
	public void eliminarListasCuotasSitPagado(Expediente expediente, Honorario honorario) 
	{	
		List<Cuota> tmpCuotas = new ArrayList<Cuota>();
		GenericDao<Cuota, Object> cuotaDAO = (GenericDao<Cuota, Object>) SpringInit.getApplicationContext().getBean("genericoDao");
		Busqueda filtro = Busqueda.forClass(Cuota.class);
		...
		filtro.add(Restrictions.eq("hno.expediente.idExpediente", expediente.getIdExpediente()));
		filtro.add(Restrictions.eq("hno.idHonorario", honorario.getIdHonorario()));
		//filtro.add(Restrictions.eq("sitCuota.descripcion", SglConstantes.SITUACION_CUOTA_PENDIENTE));
		//filtro.add(Restrictions.eq("idCuota",idCuota));
	...
	public void eliminarHonorario(Expediente expediente, Honorario honorario) 
	{	
		List<Honorario> tmpHno = new ArrayList<Honorario>();
		GenericDao<Honorario, Object> hnoDAO = (GenericDao<Honorario, Object>) SpringInit.getApplicationContext().getBean("genericoDao");
		...
		filtro.add(Restrictions.eq("idHonorario", honorario.getIdHonorario()));
		
		try {
			tmpHno=	hnoDAO.buscarDinamico(filtro);
		} catch (Exception e) {
			logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"tmpCuotas: "+e);
		}
		...
		if (isFlagEliminadoHonor()) {
			...
			for (Honorario honorario : honorarios) {
				try {
					...
					logger.error(SglConstantes.MSJ_ERROR_ELIMIN+"honorarios: ",ex);
				}
			}
		}
		...
		if (isFlagEliminadoInv()) {
			...
			for (Involucrado involucrado : involucrados) {
				try {
					...
					logger.error(SglConstantes.MSJ_ERROR_ELIMIN+"involucrados: ",ex);
				}
			}
		}
		...
		if (isFlagEliminadoCua()) {
			...
			for (Cuantia cuantia : cuantias) {
				try {
					...
					logger.error(SglConstantes.MSJ_ERROR_ELIMIN+"cuantias: ",ex);
				}
			}
		}
		...
		if (isFlagEliminadoInc()) {
			...
			for (Inculpado inculpado : inculpados) {
				try {
					...
					logger.error(SglConstantes.MSJ_ERROR_ELIMIN+"inculpado: ",ex);
				}			
			}
		}
		...
		if (isFlagEliminadoProv()) {
			...
			for (Provision provision : provisions) {
				try {
					...
					logger.error(SglConstantes.MSJ_ERROR_ELIMIN+"provisiones: ",ex);
				}
			}
		}
		...
		if (isFlagEliminadoRes()) {
			...
			for (Resumen res : resumens) {
				try {
					...
					logger.error(SglConstantes.MSJ_ERROR_ELIMIN+"resumens: ",ex);
				}
			}
		}
		...
		if (isFlagEliminadoActPro()) {
			...
			for (ActividadProcesal actividadProcesal : actividadProcesals) {
				try {
					...
					logger.error(SglConstantes.MSJ_ERROR_ELIMIN+"actividadProcesal: ",ex);
				}
			}
		}
		...
		if (isFlagEliminadoAnexo()) {
			...
			for (Anexo anexo : anexos) {						
				try {
					...
					logger.error(SglConstantes.MSJ_ERROR_ELIMIN+"anexos: ",ex);
				}
			}
		}
...
//		}
	...
	public List<Usuario> completeResponsable(String query) {
		...
		try {
			...
			logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"usuarios BD: ",e);
		}
	...
	private void cargarCombos() {
		...
		try {
			...
			logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"clases: ",e);
		}
		...
		try {
			...
			logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"tipoCautelares: "+e);
		}
		...
		try {
			...
			logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"contraCautelas: "+e);
		}
		...
		try {
			...
			logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"estadosCautelares: "+e);
		}
		...
		try {
			...
			logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"tipoProvisiones: "+e);
		}
		...
		try {
			...
			logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"situacionActProcesales: "+e);
		}
...
//		int cantHonPag=0;
		...
		for (Honorario honorario : getExpedienteVista().getHonorarios()) 
		{
			if (honorarioModif.getIdHonorario() == honorario.getIdHonorario()) {
				...
				if (total!=honorario.getTotalCuotas())
				{
					getExpedienteVista().setDeshabilitarBotonGuardar(false);
				}
				else
	...
	public Double obtenerTotalHonorario(Expediente expediente, Honorario hono){
		...
		try {
			...
			logger.error(SglConstantes.MSJ_ERROR_EXCEPTION+"al obtenerTotalHonorario: ",exCu);
		}
	...
	public int obtenerTotalCuotasxExpediente(Expediente expediente, Honorario hono){
		...
		filtro.createAlias("honorario", "hno");
		filtro.add(Restrictions.eq("hno.expediente.idExpediente", expediente.getIdExpediente()));
		filtro.add(Restrictions.eq("hno.idHonorario",hono.getIdHonorario()));
		
		try {
			tmpCuotas=	cuotaDAO.buscarDinamico(filtro);
		} catch (Exception exCu) {
			logger.error(SglConstantes.MSJ_ERROR_EXCEPTION+"al obtenerTotalCuotasxExpediente: ",exCu);
		}
		
		if (tmpCuotas!=null){
			resultado = tmpCuotas.size();
		}		
		return resultado;
	}
...
//		SituacionCuota situacionCuota = situacionCuotas.get(0);
	...
	public void editProv(RowEditEvent event) {
	 ...
	 public void editActPro(RowEditEvent event) {
		  try {
		   ...
		   logger.debug("====== saliendo de editar ActividadesProcesales ===");
					...
					if (cuota.getNumero() == cuotaModif.getNumero()) 
					{
							...
							cuotaModif.getSituacionCuota().getDescripcion().equals(SglConstantes.SITUACION_HONORARIO_BAJA)) 
						{
							...
							try {
								...
								logger.error(SglConstantes.MSJ_ERROR_CONSULTAR+"situacionCuotaTMP: "+e);
							}
		...
		try{
			...
			for (int i = 0; i < expedientes.size(); i++) {
				...
				if(modo.compareTo(ml)==0){
					setTitulo(SglConstantes.MSJ_LECTURA_EXP);
					...
					setTitulo(SglConstantes.MSJ_ACT_SEGUIM_EXP);
			...
			logger.error(SglConstantes.MSJ_ERROR_EXCEPTION +"al llenar hitos: ", e);
		}
	}
	...
	public void actualizarDatosPagina(ExpedienteVista ex, Expediente e) {
		...
		SimpleDateFormat format = new SimpleDateFormat(SglConstantes.FECH_SLASH_ddMMyyyy);
		...
		DateFormat dateFormat = new SimpleDateFormat(SglConstantes.FECH_HOR_SLASH_ddMMyyHHmmss);
		DateFormat dateFormat2 = new SimpleDateFormat(SglConstantes.FECH_SLASH_ddMMyyyy);
		...
		for (Anexo anexo : anexos) {
			...
			if (!fileTemporal.exists()) {
				...
				try {
					...
					logger.error(SglConstantes.MSJ_ERROR_EXCEPTION+"(IO) al descargar archivo: "+ anexo.getUbicacion(),ioe);
					bSaved = false;
				}catch (Exception exce){
					logger.error(SglConstantes.MSJ_ERROR_EXCEPTION+"al descargar archivo: ", exce);
					bSaved = false;
				} 
				finally {
					fichTemp.deleteOnExit(); 
				...
				if (bSaved) {
					try {
						...
						logger.error(SglConstantes.MSJ_ERROR_EXCEPTION+"al actualizar Anexo: ", exp4);
					}
				}
			} else {
	...
	public boolean isFlagNoEditar() {
		return flagNoEditar;
	}

	public void setFlagNoEditar(boolean flagNoEditar) {
		this.flagNoEditar = flagNoEditar;
	}
}
